...
import { recentlyVisited, pins } from "../db/schema.ts";
...
function ownerFilter(req) {
    if (req.userId)
        return eq(recentlyVisited.userId, req.userId);
...
router.get("/", async (req, res) => {
    try {
        ...
        const pinnedVisited = await db
            ...
            name: sql `COALESCE(${pins.customName}, ${pins.name})`,
        ...
        const unlockedNotPinned = await db
            ...
            .where(and(ownerFilter(req), eq(recentlyVisited.isPinned, false)))
...
router.post("/", async (req, res) => {
    try {
        ...
        const existing = await db.query.recentlyVisited.findFirst({
            where: and(eq(recentlyVisited.placeId, placeId), userId
