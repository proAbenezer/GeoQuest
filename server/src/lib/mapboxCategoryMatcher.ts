...
let cachedList: CategoryListItem[] | null = null
...
export async function matchMapboxCategory(input: string): Promise<MatchResult> {
  ...
  const exactCanonical = list.find(
    (item) => item.canonical_id.toLowerCase() === normalizedInput
  )
  if (exactCanonical) {
  ...
  const fuzzy = list.find(
    (item) => item.name.toLowerCase().includes(normalizedInput) || normalizedInput.includes(item.name.toLowerCase())
  )
  if (fuzzy) {
